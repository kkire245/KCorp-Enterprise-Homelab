...
## Goal
Close out Phase 0 by adding group structure and an external identity to the tenant. Both are needed as building blocks for later phases (dynamic group rules feed into access packages and CA policies in Phase 5; a guest object is needed to eventually test "require MFA for external users" style Conditional Access).
...
## Why This Way
Went with a job-title-based rule ("contains Manager") for the dynamic group rather than a department-based one, since department-based dynamic groups would just duplicate the assigned department groups I already made. A cross-department "all managers" group is a more realistic use case, the kind of group that would back a real access package or approval-workflow scenario later (e.g., "managers can approve access requests"), and it's a better demonstration that dynamic membership actually works across the org, not just within one filter.

The guest invite is a small step now but sets up future work, external users are a distinct identity type in Entra with different default access rules, and several later Conditional Access scenarios in the plan (Stage 6) specifically target guest/external accounts.
